...
// Copyright (c) 2013 The SecondCoin developers
...
void StakeMiner(CWallet *pwallet)
{
    ...
    RenameThread("SecondCoin-Stake-miner");
...
//
// PoW mining part will be removed from the official release
//
...
static bool fGenerateSecondCoins = false;
...
void SecondCoinMiner(CWallet *pwallet)
{
    ...
    // Make this thread recognisable as the mining thread
    RenameThread("SecondCoin-PoW-miner");

    // Each thread has its own key and counter
    CReserveKey reservekey(pwallet);
    unsigned int nExtraNonce = 0;

    while (fGenerateSecondCoins)
    {
        if (fShutdown)
            return;
        while (vNodes.empty() || IsInitialBlockDownload())
        ...
        printf("Running SecondCoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void static ThreadSecondCoinMiner(void* parg)
{
    CWallet* pwallet = (CWallet*)parg;
    try
    {
        vnThreadsRunning[THREAD_MINER]++;
        SecondCoinMiner(pwallet);
        vnThreadsRunning[THREAD_MINER]--;
    }
    catch (std::exception& e) {
        vnThreadsRunning[THREAD_MINER]--;
        PrintException(&e, "ThreadSecondCoinMiner()");
    } catch (...) {
        vnThreadsRunning[THREAD_MINER]--;
        PrintException(NULL, "ThreadSecondCoinMiner()");
    }
    printf("ThreadSecondCoinMiner exiting, %d threads remaining\n", vnThreadsRunning[THREAD_MINER]);
}

void GenerateSecondCoins(bool fGenerate, CWallet* pwallet)
{
    fGenerateSecondCoins = fGenerate;
    nLimitProcessors = GetArg("-genproclimit", -1);
    if (nLimitProcessors == 0)
        fGenerateSecondCoins = false;
    bool fLimitProcessors = (nLimitProcessors != -1);
    ...
    if (fGenerate)
    {
        ...
        printf("Starting %d SecondCoinMiner threads\n", nAddThreads);
        for (int i = 0; i < nAddThreads; i++)
        {
            if (!NewThread(ThreadSecondCoinMiner, pwallet))
                printf("Error: NewThread(ThreadSecondCoinMiner) failed\n");
            Sleep(10);
        }
    }
}
